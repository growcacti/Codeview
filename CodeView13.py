...
##        self.f1.grid_columnconfigure(0, weight=1)  # Expand first column
        ...
        self.dirfr.grid(row=0, column=1, sticky="nw")  # Place in first row, first column
       
        tk.Label(self.dirfr, text="--------------").grid(row=1, column=0, pady=5)

        self.fram = tk.Frame(self.f1, width=20, height=20)
        self.fram.grid(row=0, column=1, pady=10,padx=(0,4),sticky="w")  # Place in second row, first column
        ...
        self.tx = ScrolledText(self.textfr,
                      bg='white',
                      bd=12,
                      relief=GROOVE,
                      height=44,
        ...
        self.fram2.grid(row=0, column=1, padx=(0,4),sticky="w")  # Place in second row, first column
        self.fram2.columnconfigure(1, weight=4, minsize=55)
        self.lb2 = tk.Listbox(self.fram2, bg='cyan2', bd=12, width=35, height=45, exportselection=False, selectmode=tk.SINGLE)
        self.lb2.grid(row=0, column=1, rowspan=2,padx=(0,2), sticky="nw")  # Place in first row, first column with rowspan
      # self.lb2.columnconfigure(1, weight=0, minsize=55)
        #self.fram2.columnconfigure(1, weight=4, minsize=55)
        self.dirfr2 = ttk.Frame(self.f2, width=50, height=20)
        self.dirfr2.grid(row=0, column=1, sticky="nw")  # Place in first row, second column
        ...
        self.dirpath2.insert(INSERT, self.path)
        ...
        self.tttx = ScrolledText(self.textfr2,
                      bg='white',
                      bd=12,
                      relief=GROOVE,
                      height=44,
...
##        btnbtn.grid(row=4, column=3)
        self.out= ScrolledText(self.f7,bg='white', bd=12,
                      relief=GROOVE,
                      height=50,
                      width=100,
                      font='TkFixedFont',)
        ...
        self.cvw2 = Cview_Buttons(self.f2,self.tttx,self.lb2, self.txxx, self.dirpath2)


       
        self.lb2.bind("<Double-Button-1>", self.cvw2.listing)
        self.lb2.bind("<<ListboxSelect>>", self.cvw2.showcontent)
        self.lb2.bind("<Double-Button-2>", lambda event: self.cvw2.run(self.path,self.lb2))
        ...
        self.lb.bind("<Double-Button-2>", lambda event: self.cvw.run(self.path,self.lb))
...
class SideButtonframe:
    def __init__(self,parent,textwidget):
        ...
        self.fr_buttons = tk.Frame(self.parent, width=5)
    ...
    def update_info(self, event=None):
        ...
        self.info_label = tk.Label(self.fr_buttons, text="Lines: 0 | Words: 0 |\n Characters: 0")
    ...
    def open_file(self,textwidget):
        ...
        filepath = filedialog.askopenfilename(initialdir=os.path.expanduser("/Desktop"), title="Select File", filetypes=(("Python files", "*.py"), ("All files", "*.*")))
        if filepath:
    ...
    def insert_self(self, textwidget):
        ...
        modified_text = "\n".join(["self." + line if line.strip() else line for line in selected_text.split("\n")])
    ...
    def insert_selfs(self, textwidget):
        ...
        selected_text = self.textwidget.get(start_index, end_index)
        modified_lines = ["self." + line.strip() if line.strip() else line for line in selected_text.split("\n")]
        modified_text = "\n".join(modified_lines)
...
class Cview_Buttons(SideButtonframe):
    def __init__(self, container,textwidget, lb, tx2,dirpath):
        ...
        self.btn_run = tk.Button(self.fr_buttons, text="run py ",bd=4, bg= "light green", command=lambda : self.run(self.path,self.lb))
        self.btn_run.grid(row=25, column=0)
        self.btn_run2 = tk.Button(self.fr_buttons, text="run py2 ",bd=4, bg= "light green", command=lambda : self.run2(self.path,self.lb))
...
class TextFileComparator:
    ...
    def create_gui(self):
        ...
        compare_button = tk.Button(self.btfr, text="Compare", command=self.compare_files)
    ...
    def update_info(self, event=None):
        ...
        words = len(self.content.split())
        characters = len(self.content)
        self.info_label = tk.Label(self.master, text="Lines: 0 | Words: 0 |\n Characters: 0")
        self.info_label.grid(row=20, column=0)
        lines = self.text2.index("end-1c").split(".")[0]
        words = len(self.content.split())
        characters = len(self.content)
        self.info_label2 = tk.Label(self.master, text="Lines: 0 | Words: 0 |\n Characters: 0")
