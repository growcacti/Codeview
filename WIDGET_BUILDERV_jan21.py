...
class CodeEditorApp:
    def __init__(self, root):
        ...
        self.max_columns = (
            3  # Example: Limit to 3 columns before moving to the next row
        )

        # Boilerplate code
        self.prefix_boilerplate = """\nimport tkinter as tk\nfrom tkinter import ttk\n\nroot = tk.Tk()\ndef main():\n"""
    ...
    def create_toplevel(self):
        ...
        if self.top is None or not self.top.winfo_exists():  # Check if Toplevel exists
            ...
            self.top.protocol(
                "WM_DELETE_WINDOW", self.on_toplevel_close
            )  # Handle close event
    ...
    def setup_ui(self):
        ...
        tk.Label(selection_frame, text="Widget Type:").grid(
        ...
        # Text Entry
        tk.Label(selection_frame, text="Text:").grid(
        ...
        # Widget Width
        tk.Label(selection_frame, text="Width:").grid(
        ...
        # Widget Height
        tk.Label(selection_frame, text="Height:").grid(
        ...
        # Relief Style
        tk.Label(selection_frame, text="Relief:").grid(
        ...
        tk.Label(selection_frame, text="Background (bg):").grid(
            row=5, column=0, sticky="w", pady=2
        )
        self.bg_var = tk.StringVar(value="white")
        ttk.Combobox(
            selection_frame, textvariable=self.bg_var, values=self.colors
        ).grid(row=5, column=1, sticky="ew", pady=2)

        tk.Label(selection_frame, text="Foreground (fg):").grid(
            row=6, column=0, sticky="w", pady=2
        )
        self.fg_var = tk.StringVar(value="black")
        ttk.Combobox(
            selection_frame, textvariable=self.fg_var, values=self.colors
        ).grid(row=6, column=1, sticky="ew", pady=2)

        # Border Width
        tk.Label(selection_frame, text="Border Width (bd):").grid(
            row=7, column=0, sticky="w", pady=2
        )
        self.bd_var = tk.IntVar(value=2)
        tk.Entry(selection_frame, textvariable=self.bd_var).grid(
        ...
        clear_preview_button = tk.Button(
            selection_frame,
            bd=5,
            bg="thistle",
            text="Clear Preview",
        ...
        tk.Button(
            action_frame, text="Insert Boilerplate", command=self.insert_boilerplate
        ).grid(row=0, column=0, padx=5)
        tk.Button(action_frame, text="Clear All", command=self.clear_all).grid(
            row=0, column=1, padx=5
        )
        tk.Button(
            action_frame, text="Copy to Clipboard", command=self.copy_to_clipboard
        ).grid(row=0, column=2, padx=5)
        tk.Button(action_frame, text="Save to File", command=self.save_to_file).grid(
            row=0, column=3, padx=5
        )
        tk.Button(
            action_frame, text="Combine Code and Run", command=self.combine_and_run
    ...
    def generate_code(self):
        ...
        if widget_type == "Entry":
            ...
            widget = ttk.Combobox(
                self.preview_area, values=["Option 1", "Option 2"], width=width
            )
