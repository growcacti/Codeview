...
class CodeFormatter:
    ...
    def _create_original_code_tab(self):
        ...
        self.text_original = ScrolledText(
            tab_original, bd=7, wrap="word", width=160, height=40)
        self.text_original.grid(row=0, column=0, sticky="nsew", padx=5, pady=5)

        btn_load = tk.Button(tab_original, text="Load Code",
                             command=self.load_file)
        btn_load.grid(row=1, column=0, pady=5)
        btn_clear = tk.Button(
            tab_original, text="Clear Code", command=self.clear_orig)
    ...
    def _create_formatted_code_tab(self):
        ...
        self.text_formatted = ScrolledText(
            tab_formatted, bd=11, wrap="word", state="normal", width=160, height=40)
        self.text_formatted.grid(
            row=0, column=0, columnspan=2, sticky="nsew", padx=5, pady=5)

        btn_black = tk.Button(tab_formatted, fg="white", bg="black",
                              bd=7, text="Format with Black", command=self.format_with_black)
        btn_black.grid(row=1, column=0, padx=5, pady=5)

        btn_autopep8 = tk.Button(tab_formatted, bd=5, bg="hot pink",
                                 text="Format with Autopep8", command=self.format_with_autopep8)
        btn_autopep8.grid(row=1, column=1, padx=5, pady=5)

        btn_save = tk.Button(tab_formatted, bd=6, bg="orange",
                             text="Save Code", command=self.save_formatted_code)
        btn_save.grid(row=2, column=0, columnspan=2, pady=5)
        btn_wipe = tk.Button(tab_formatted, bd=7, bg="plum",
                             text="Clear Code", command=self.clear_formatted)
    ...
    def _create_linter_output_tab(self):
        ...
        self.text_linter = ScrolledText(
            tab_linter, bd=11, wrap="word", state="normal", width=160, height=40)
        self.text_linter.grid(row=0, column=0, sticky="nsew", padx=5, pady=5)

        btn_lint = tk.Button(tab_linter, bd=6, bg="wheat",
                             text="Run Linter", command=self.run_linter)
        btn_lint.grid(row=1, column=0, pady=5)
        self.clearlint = tk.Button(
            tab_linter, bd=8, bg="light green", text="Clear Linter", command=self.clear_linter)
    ...
    def save_formatted_code(self):
        ...
        if self.original_file_path:
            base_name = self.original_file_path.rsplit(
                "/", 1)[-1].rsplit(".", 1)[0]
        else:
        ...
        if save_path:
            ...
            messagebox.showinfo(
                "Success", f"Formatted code saved to {save_path}!")
    ...
    def _format_with_black(self, code):
        ...
        try:
            formatted_code = format_file_contents(
                code, fast=True, mode=FileMode())
    ...
    def _lint_code(self, code):
        ...
        try:
            Run(['--from-stdin'], do_exit=False,
                stdout=pylint_output, stdin=StringIO(code))
