...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.scan_type_label = ttk.Label(
            self.directory_tab, text="Select Scan Type:")
        ...
        self.directory_radio = ttk.Radiobutton(
            self.directory_tab, text="Directory", variable=self.scan_type_var, value="directory")
        self.file_radio = ttk.Radiobutton(
            self.directory_tab, text="Single File", variable=self.scan_type_var, value="file")
        ...
        self.directory_label = ttk.Label(
            self.directory_tab, text="Select a Directory or File:")
        ...
        self.hierarchy_tree.grid(
            row=0, column=0, padx=10, pady=10, sticky="nsew")

        # Module Info tab widgets
        self.module_info_label = ttk.Label(
            self.module_info_tab, text="Module Details will appear here")
        self.module_info_label.grid(
            row=0, column=0, padx=10, pady=10, sticky="w")

        # Lib Info tab widgets
        self.lib_info_label = ttk.Label(
            self.lib_info_tab, text="Library Info will appear here")
        self.lib_info_label.grid(row=0, column=0, padx=10, pady=10, sticky="w")

        # Save button
        self.save_button = ttk.Button(
            self, text="Save Results", command=self.save_results)
    ...
    def scan_directory_or_file(self):
        ...
        if self.scan_type_var.get() == "directory":
            ...
            if not os.path.isfile(selected_path):
                messagebox.showerror(
                    "Error", "Please select a valid Python file.")
                return
    ...
    def display_directory_structure(self, directory):
        ...
        for root, dirs, files in os.walk(directory):
            parent = self.directory_tree.insert(
                "", "end", text=root, open=True)
            for file in files:
                if file.endswith(".py"):
    ...
    def display_hierarchy(self):
        ...
        for file, classes_methods in self.scanner.classes_methods.items():
            parent = self.hierarchy_tree.insert(
                "", "end", text=file, open=True)
            for class_name, methods in classes_methods.items():
                class_item = self.hierarchy_tree.insert(
                    parent, "end", text=class_name, open=True)
