...
class MultiTabTextEditor:
    def __init__(self, parent):
        ...
        self.add_tab()  
        # Menu bar
        self.menubar = tk.Menu(self.parent,background ="SkyBlue2")
        self.parent.config(menu=self.menubar)
        parent.option_add("*Menu.font", "Verdana 8 bold")
        # File menu
        ...
        self.format_menu.add_command(label = "Code Formatter & Lint", command = self.add_mat_tab)
        ...
        self.custom_menu.add_command(
            label="Correct_Sentances", command=self.correct_sentences)
    ...
    def quit(self):
        if self.parent.winfo_exists():  # Check if the window still exists
            try:
    ...
    def add_app_tab(self):
        aootab = tk.Frame(self.notebook)
        apptab.grid_rowconfigure(0, weight=1)
        apptab.grid_columnconfigure(0, weight=1)
        self.notebook.add(apptab, text=f"Tab {len(self.tabs) + 1}")
        self.tabs.append(self.textwidget)
        self.notebook.select(len(self.tabs) - 1)

    def add_mat_tab(self):
        mattab = tk.Frame(self.notebook)
        mattab.grid_rowconfigure(0, weight=1)
        mattab.grid_columnconfigure(0, weight=1)
        self.notebook.add(mattab, text=f"Code Formater {len(self.tabs) + 1}")
        self.formatcode = mat.CodeFormatter(mattab)
        self.tabs.append(self.formatcode)
        self.notebook.select(len(self.tabs) - 1)
        btn = tk.Button(mattab,bd=7,bg="light pink",text="send code",command = self.get_cur_code)
    ...
    def get_cur_code(self):
        self.sendcode = self.textwidget.get("1.0", tk.END)
        self.formatcode.text_original.insert("1.0", self.sendcode)
    def run_file(self):
    ...
    def run_py_file(self):
        ...
        current_text_widget = self.notebook.nametowidget(current_tab)
        file_name = self.notebook.tab(current_tab, "text")  # Get the tab's title
        if file_name == "Untitled" or file_name is None:
            self.save_file_as() 
        else:
    ...
    def show_execution_output(self, stdout, stderr):
        ...
        self.parent.wait_window(font_choice)
    ...
    def correct_sentences(self):
        text = self.textwidget.get("1.0", tk.END)  # Get all text from the Text widget
        ...
        self.textwidget.insert(tk.END, corrected_text)  # Insert corrected text
    ...
    def choose_font(self):
        FontBar(self.parent, self.textwidget)
    ...
    def _search_files(self):
        for parent, dirs, files in os.walk(self.path):
            for file in files:
                if any(file.endswith(ext) for ext in self.file_extensions):
                    self.lb.after(0, self.lb.insert, tk.END, os.path.join(parent, file))
