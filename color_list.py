...
picked = tk.StringVar()
picked.set("blue")
f7 = ttk.Frame(root, height=60, width=400)
f7.grid(row=1, column=1)
cbc = ttk.Combobox(root, values=colors, textvariable=picked)
cbc.grid(row=4, column=1)
cbc.bind
bgg = picked.get()
bg1 = picked
btn1 = tk.Button(root,text="select", command=picked.get())
btn1.grid(row=5, column=1)
can= Canvas(root, bg=[bgg], height=10, width=10).grid(row=5, column=2)
...
cbc.bind("<<ComboboxSelected>>" ,picked.get())
